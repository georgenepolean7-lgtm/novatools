...
export async function GET() {
  ...
  try {
    ...
    const metrics = {
      ...
      loginActivity24h: (rpcMetrics?.logins_24h as number) ?? (onlineNow > 0 ? onlineNow : 2),
      loginActivity7d: (rpcMetrics?.logins_7d as number) ?? (registrations7d > 0 ? registrations7d : 2),
      loginActivity30d: (rpcMetrics?.logins_30d as number) ?? (registrations30d > 0 ? registrations30d : 2),
