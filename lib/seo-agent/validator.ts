...
export class SeoValidator {
  ...
  validatePageStageA(slug: string, postPatchOverrides: PostPatchMetadataOverrides = {}): StageAValidationResult {
    ...
    const effectiveTitle = (postPatchOverrides.seoTitle ?? tool.seoTitle ?? "").trim();
    const effectiveDescription = (postPatchOverrides.seoDescription ?? tool.seoDescription ?? "").trim();
    ...
    if (!effectiveTitle || effectiveTitle.length > meta.MAX_TITLE_LENGTH ||
        (postPatchOverrides.titleWasPatched ? effectiveTitle.length < meta.MIN_TITLE_LENGTH : false)) {
      return {
        passed: false,
        slug,
        ...
        durationMs: Date.now() - start,
        checks: [...checks, { name: "Metadata Invariants Gate", passed: false, message: `seoTitle length out of bounds: ${effectiveTitle.length}` }],
      };
    }
    if (!effectiveDescription || effectiveDescription.length > meta.MAX_DESCRIPTION_LENGTH ||
        (postPatchOverrides.descWasPatched ? effectiveDescription.length < meta.MIN_DESCRIPTION_LENGTH : false)) {
      return {
        passed: false,
        slug,
        ...
        durationMs: Date.now() - start,
        checks: [...checks, { name: "Metadata Invariants Gate", passed: false, message: `seoDescription length out of bounds: ${effectiveDescription.length}` }],
      };
    }
    checks.push({
    ...
    const factualCheck = FactualContentSafetyValidator.validate(tool, "CONTENT_UPDATE", {
      ...
      faqs: postPatchOverrides.faqs ?? tool.faq,
